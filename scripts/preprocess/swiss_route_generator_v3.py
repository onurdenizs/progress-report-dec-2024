...
def parse_network_file(network_file):
    ...
    try:
        ...
        for edge in root.findall("edge"):
            edge_id = edge.get("id")
            if edge_id:
                match = re.match(r"edge_([0-9]+_[A-Z]+_[A-Z]+).*", edge_id)
                if match:
                    pattern = match.group(1)  # Extract pattern like `500_BS_BSO`
                    edge_map.setdefault(pattern, []).append(edge_id)

        logger.info(f"Parsed network with {len(edge_map)} unique patterns.")
        logger.debug(f"Edge patterns: {list(edge_map.keys())[:10]}...")  # Log a sample of patterns
        return edge_map
...
def generate_route_edges(stations, edge_map):
    ...
    for i in range(len(stations) - 1):
        ...
        pattern = f":{station1}_{station2}"
        edges = edge_map.get(pattern, [])

        if not edges:
            logger.warning(f"No edges found for pattern {pattern}. Skipping segment.")
        else:
            logger.debug(f"Edges found for pattern {pattern}: {edges}")
...
def generate_routes_with_stops(linenr, stations, edge_map, waiting_stations, wait_time_range, speed_range, min_depart_time, max_depart_time, min_depart_gap, output_file):
    ...
    for train_num in range(1, 6):
        ...
        vehicle_elem = ET.SubElement(root, "vehicle", id=f"train_{linenr}_{train_num}", type="defaultTrain", route=route_id, depart=str(depart_time), maxSpeed=str(speed))
        ...
        for stop_station in waiting_stations:
            if stop_station in stations:
                ...
                if stop_idx > 0:
                    ...
                    pattern = f"500_{station1}_{station2}"
                    edges = edge_map.get(pattern, [])
                    if edges:
                        stop_edge = edges[-1]
